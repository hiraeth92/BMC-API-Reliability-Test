...
import tempfile  # 【CI 關鍵修正】用來獲取跨平台的暫存目錄，解決 Windows/Linux 環境變數差異問題。

# --- VQE Log 配置：專為問題追蹤設計 ---
# 這是 Log 系統的核心配置，確保無論測試環境多複雜（例如在 Pytest 或 Windows/Linux），
# 我們的錯誤追蹤和報告都能正常運作。

# 1. 定義 Log 檔案路徑
# 【CI 修正】使用 tempfile.gettempdir() 確保在 Windows (%TEMP%) 和 Linux/GitHub Actions (/tmp) 都能正確找到暫存目錄。
LOG_FILENAME = os.path.join(tempfile.gettempdir(), 'reliability_errors.log')
# 每次運行前，先清除舊的 Log 檔案，確保報告是最新一輪的。
...
# 確保測試運行時，數據能在螢幕上即時顯示。
# 註：在 CI 環境中，Pytest 可能會捕獲此輸出，因此我們在測試函數中使用了 print() 作為保險。
stream_handler = logging.StreamHandler()
stream_handler.setFormatter(formatter)
logger.addHandler(stream_handler)

# 5. 手動創建 FileHandler (輸出到檔案，這是我們正式的 VQE 報告)
# 這是最關鍵的部分。我們強制指定 'utf-8' 編碼來避免編碼錯誤。
...
class TestApiReliability:
    ...
    def test_performance_statistics_analysis(self, concurrent_test_results):
        ...
        if len(successful_latencies) < 2:
            # 如果成功請求數不足，則跳過效能分析
            logger.warning("成功請求數不足，無法進行統計分析。")
        ...
        # 將報告組合成一個字串 (包含 CI 成功或失敗訊息)
        is_passed = average_latency < LATENCY_THRESHOLD_MS
        status_line = "✅ 效能測試通過" if is_passed else "❌ 效能測試失敗"

        report_text = f"""
--- 效能統計數據報告 ---
目標 API: {TARGET_URL}
併發請求數: {NUM_REQUESTS}
平均延遲 (Avg Latency): {average_latency:.2f} ms
標準差 (Std Dev): {std_dev_latency:.2f} ms (VQE 穩定性指標)
P95 延遲: {p95_latency:.2f} ms
效能閾值 (Threshold): {LATENCY_THRESHOLD_MS} ms
---------------------------
{status_line}
"""

        # 1. 將報告寫入 Log 檔案 (VQE 官方報告)
        # 使用 logger.info() 寫入檔案，確保包含時間戳和 Log 等級
        logger.info(report_text.strip())

        # 2. 【CI 終端機輸出修正】使用 print() 確保在 Pytest/GitHub Actions 捕獲模式下也能立即顯示。
        print(report_text)
